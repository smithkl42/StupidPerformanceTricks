...
namespace Alanta.Client.Controls.AudioVisualizer
{
    public class FourierTransform
    {
        ...
        public static float[] FFTDb(ref float[] x)
        {
            //uint pow2Samples = FFT.NextPowerOfTwo((uint)x.Length);
            float[] xre = new float[x.Length];
            float[] xim = new float[x.Length];

            Compute((uint)x.Length, x, null, xre, xim, false);

            float[] decibel = new float[xre.Length / 2];

            for (int i = 0; i < decibel.Length; i++)
                decibel[i] = (float)(10.0 * Math.Log10((float)(Math.Sqrt((xre[i] * xre[i]) + (xim[i] * xim[i])))));
            return decibel;
        }
        ...
        public static float PeakFrequency(UInt32 NumSamples, float[] pAmpl, float samplingRate, ref UInt32 index)
        {
            ...
            for (UInt32 i = 0; i < N; i++)
            {
                if (pAmpl[i] > maxAmpl)
                {
                    maxAmpl = (float)pAmpl[i];
                    index = i;
                    peakFreq = (float)(i);
                }
            }

            return samplingRate * peakFreq / (float)(NumSamples);
        }

        public static byte[] GetPeaks(float[] leftChannel, float[] rightChannel, int sampleFrequency)
        {
        ...
        private static byte[] GetPeaksForChannel(float[] normalizedArray, int sampleFrequency)
        {
            float maxAmpl = (float)(32767.0f * 32767.0f);
