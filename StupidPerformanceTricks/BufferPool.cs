...
namespace StupidPerformanceTricks
{
	...
	public class BufferPool<T> where T:class
	{
		...
		public BufferPool(Func<T> newFunction)
		{
			stack = new Stack<T>();
		...
		public BufferPool(Func<T> newFunction, Action<T> resetAction)
			: this(newFunction)
		{
		...
		public virtual T GetNext()
		{
			...
			lock (stack)
			{
				if (stack.Count == 0)
				{
					...
					{
						ClientLogger.LogDebugMessage("Object Pool for type {0}: Created={1}; Reused={2}; Recycled={3}; Outstanding={4}",
